...
#        jv_url = i['date']
        if int(i['has_video']) == 1:
            dump[0]['has_video'] = '<img src="images/filmreel.png" style="width:32px;height:32px;">'
            ...
            print(dump[0]['has_video'])
        #            jv = '<a href="journal.php?date='+ str(jv_url) +'" ><img src="images/filmreel.png" style="width:32px;height:32px;"></a>'
        else:
            jv = '--'
            dump.append({'id': i['id'], 'date': i['date'], 'ice_time': i['ice_time'], 'ice_cost': i['ice_cost'],
                         'skate_type': i['skate_type'], 'coach_time': i['coach_time'], 'coach_id': i['coach_id'],
                         'rink_id': i['rink_id'], 'has_video': jv, 'has_notes': i['has_notes'],
                         'coach_fname': i['coach_fname'], 'coach_lname': i['coach_lname'],
                         'coach_rate': i['coach_rate'], 'location_id': i['location_id'],
                         'location_city': i['location_city'], 'location_state': i['location_state'], 'type': i['type']})
